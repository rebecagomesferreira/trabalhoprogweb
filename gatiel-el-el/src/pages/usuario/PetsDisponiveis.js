...
function PetsDisponiveis() {
  ...
  async function handleSolicitar() {
    ...
    try {
      // ✅ Proteção contra solicitação duplicada:
      // Verifica se já existe um requerimento Pendente deste usuário para este pet
      const q = query(
        collection(db, 'requerimentos'),
        where('usuarioId', '==', usuario.id),
        where('petId',     '==', modalPet.id),
        where('status',    '==', 'Pendente')
      );
      const jaExiste = await getDocs(q);
      if (!jaExiste.empty) {
        setErro('Você já enviou uma solicitação para este pet. Aguarde a resposta do administrador.');
        setEnviando(false);
        return;
      }

      // Grava o requerimento normalmente

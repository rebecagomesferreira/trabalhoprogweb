...
function Usuarios() {
  ...
  useEffect(() => {
    async function carregar() {
      ...
      // Filtra para não exibir usuários comuns (role: 'user'), apenas admin e usuários sem role definido
      const filtrados = todos.filter(u => !u.role || u.role === 'admin');
      setUsuarios(filtrados);
    }
    carregar();
  }, []);

  // CREATE — com validação de duplicação por e-mail
  async function handleCadastrar(e) {
    ...
    // ✅ Validação: verifica se já existe usuário com este e-mail
    ...
    const novo = { nome, email: email.trim(), senha, telefone, role };
  ...
  // UPDATE — com validação de duplicação de e-mail (exceto o próprio usuário)
  async function handleAtualizar(e) {
    ...
    // ✅ Validação: verifica se outro usuário já tem este e-mail
    if (email !== editando.email) {
      const q = query(collection(db, 'usuarios'), where('email', '==', email.trim()));
      const snapshot = await getDocs(q);
      if (!snapshot.empty) {
        setErro('Este e-mail já está cadastrado por outro usuário.');
        return;
      }
    }

    const dados = { nome, email: email.trim(), senha, telefone, role };
